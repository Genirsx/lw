...
毕业设计项目，当前版本采用 `React + Vite + Go + MySQL + Hardhat + Solidity`。系统支持公益项目展示、捐赠记录存证、拨付追踪、链上校验、管理员审核，以及“项目申请者提交项目，管理员审核后上线”的流程。
...
Go 后端启动时会自动补齐这 3 组演示账号。登录页也支持一键填充它们。
...
- `JWT_SECRET`：登录签名密钥
- `DB_CLIENT`：当前固定为 `mysql`
- `MYSQL_HOST` / `MYSQL_PORT` / `MYSQL_USER` / `MYSQL_PASSWORD` / `MYSQL_DATABASE`：MySQL 连接配置
- `DB_FILE`：旧 SQLite 文件路径，仅在执行迁移命令时使用，默认 `server/data/app.db`
...
- 当前正式后端使用 `MySQL`
- 本地开发默认通过 `docker-compose.yml` 启动 `mysql:8.4`
- Go 服务启动时会自动建表并补齐演示账号
- 如果要把旧的 SQLite 数据迁入 MySQL，请执行 `npm run db:migrate:mysql`
...
- SQLite 演示说明：`SQLITE_DEMO.md`（旧版迁移参考）
- 系统说明与答辩梳理：`docs/ARCHITECTURE.md`
- 概要设计说明：`docs/SUMMARY_DESIGN.md`
